...
struct PerftTest {
    ...
    void operator()() {
        ...
        for (int i = 0; i < expectedNodes.size(); ++i) {
            ...
            ui64 result = perft(pos, depth, false);

            LUNA_ASSERT(result == expected,
                        "Expected " << expected << ", got " << result << " for perft " << depth
                                    << " at pos '" << fen << "'");
        }
    }
};

std::vector<TestCase> perftTests = {
    ...
    PerftTest("n1n5/PPPk4/8/8/8/8/4Kppp/5N1N b - - 0 1", { 24, 496, 9483, 182838, 3605103, 7117913 ,}),
    PerftTest("r1b1r3/1pp2p1p/3p4/2b2Pk1/p1PPp1Pn/P6R/1P2BP1P/R1B1K3 b Q d3 0 22", { 2, 50, 1519, 36092, 1079212, 26548999 ,})
};
